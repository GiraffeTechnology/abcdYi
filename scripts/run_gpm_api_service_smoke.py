...
from src.gpm.api.deps import _try_build_service, get_runtime_config  # noqa: E402
_try_build_service.cache_clear()
get_runtime_config.cache_clear()

_api_key = os.environ.get("GPM_API_KEY", "").strip()
...
def main() -> None:
    ...
    r = client.post("/api/gpm/quote-guidance", headers=_auth_headers, json={
    ...
    if check("POST /api/gpm/quote-guidance", r.status_code == 201, r.text[:120]):
        ...
        check(f"GET /quote-guidance/{pid}",
              client.get(f"/api/gpm/quote-guidance/{pid}", headers=_auth_headers).status_code == 200)

        apr = client.post(f"/api/gpm/quote-guidance/{pid}/approve", headers=_auth_headers, json={
        ...
        r2 = client.post("/api/gpm/quote-guidance", headers=_auth_headers, json={"rfq_id": "smoke-rfq-002"})
        if r2.status_code == 201:
            pid2 = r2.json()["packet"]["packet_id"]
            check("POST .../reject",
                  client.post(f"/api/gpm/quote-guidance/{pid2}/reject", headers=_auth_headers,
