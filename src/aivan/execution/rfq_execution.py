...
def interpret_strategy(raw_text: str, context: Optional[GiraffeContext] = None) -> RFQStrategy:
...
def _create_rfq_from_event_inner(event: OpenClawEvent, db: Session) -> RFQExecutionResult:
    ...
    giraffe_db_graph_error: Optional[dict] = None
    try:
...
def _load_requirement(payload: Optional[dict]) -> Optional[BuyerRequirement]:
    if not payload:
        return None
    try:
