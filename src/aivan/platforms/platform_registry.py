...
def get_platform(platform_id: str) -> Optional[TrustedPlatform]:
    _ensure_init()
...
def update_platform_status(platform_id: str, status: str) -> Optional[TrustedPlatform]:
    _ensure_init()
    with _lock:
        p = _platforms.get(platform_id)
...
def approve_suggestion(suggestion_id: str) -> Optional[PlatformSuggestion]:
    _ensure_init()
...
def reject_suggestion(suggestion_id: str) -> Optional[PlatformSuggestion]:
    _ensure_init()
    sug = _suggestions.get(suggestion_id)
    if sug:
        sug.status = "rejected"
...
def block_suggestion(suggestion_id: str) -> Optional[PlatformSuggestion]:
    _ensure_init()
    sug = _suggestions.get(suggestion_id)
    if sug:
        sug.status = "blocked"
